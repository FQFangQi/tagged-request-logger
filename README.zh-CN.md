...
*   **🖱️ 精准 DOM 路径追踪**：自动获取点击元素的唯一 CSS Selector 路径（支持 `nth-of-type` 计算兄弟节点位置），完美指导 AI 确定被操作的 DOM。
...
*   **GitHub 快速直达**：在配置页面内置了一个明显的开源仓库直达按钮，方便前往主页提交 Issue 或获取更新。
*   **可视化使用指南**：在设置页内嵌入了一个纯 CSS 绘制的垂直时间轴步骤条，直观指引录制、打标、复制到与 AI 交互的完整工作流。
*   **检查更新与手动同步**：集成交互式版本比对卡片。能够拉取 GitHub 上的最新 Manifest 并比对当前本地版本，对浏览器 CORS/CSP 安全阻拦做了优雅降级，并提供一键获取最新 Zip 包的手动更新通道。
