    ...
    <div id="brl-preview-list">
      {displayLogs.map(({ log, index }) => {
        ...
        if (log.type === 'click') {
          ...
          return (
            <div key={index} className="brl-log-row status-click" onClick={() => handleRowClick(log)}>
        ...
        if (log.type === 'error' || log.type === 'promise-error') {
          ...
          return (
            <div key={index} className="brl-log-row status-err" onClick={() => handleRowClick(log)}>
        ...
        if (log.type === 'console') {
          ...
          return (
            <div key={index} className={`brl-log-row ${rowClass}`} onClick={() => handleRowClick(log)}>
              <div style={{ flex: 1, textAlign: 'left', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '6px', paddingRight: '46px', overflow: 'hidden' }}>
                <span>{icon}</span>
